...
func (repository *LessonRepository) GetLessonsByChapterId(chapterID uint) ([]*domain.Lesson, error) {
	var lessons []*domain.Lesson
	if err := repository.database.Where("chapter_id = ?", chapterID).Find(&lessons).Error; err != nil {
		return nil, err
	}
	return lessons, nil
}
...
func (repository *LessonRepository) UpdateLesson(lesson *domain.Lesson) (*domain.Lesson, error) {
	updatedLesson, err := repository.Update(lesson)
...
func (repository *LessonRepository) DeleteLesson(lesson *domain.Lesson) error {
	_, err := repository.Delete(lesson)
